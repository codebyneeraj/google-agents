...
def get_soc_tool_declarations() -> List[Dict[str, Any]]:
    ...
    return [
        ...
        {
            "name": "isolate_host",
            "description": "Quarantines a compromised host machine or blocks an attacking IP address from the network.",
            "parameters": {
                "type": "OBJECT",
                "properties": {
                    "host_id": {
                        "type": "STRING",
                        "description": "The hostname, server ID, or attacker IP address to block."
                    },
                    "reason": {
                        "type": "STRING",
        ...
        {
            "name": "inspect_linux_auth_logs",
            "description": "Inspects live Linux authentication logs (/var/log/auth.log or journalctl) for brute-force telemetry.",
            "parameters": {
                "type": "OBJECT",
                "properties": {
                    "ip_address": {
                        "type": "STRING",
                        "description": "Optional IP address to filter log events."
                    },
                    "max_lines": {
                        "type": "INTEGER",
                        "description": "Maximum number of recent lines to inspect (default 50)."
                    }
                }
            }
        }
